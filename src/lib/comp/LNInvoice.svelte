<script lang="ts">
    ...
    import { copyTextToClipboard, createOutputAmount, createOutputAmounts, delay } from "../utils";
    ...
    const createTokensForPrint = async (
        ...
        while (ps.length) {
            ...
            for (const amount of outputAmount) {
                const proof = ps.splice(ps.findIndex(p=>p.amount===amount),1)
...
<!-- content here -->
 <div class="flex flex-col gap-2 items-center">
    ...
    <div class="flex items-center gap-2 w-full">
        ...
        <button class="btn btn-primary" onclick={()=> copyTextToClipboard(invoice)}>
