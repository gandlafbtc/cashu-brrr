import { CashuMint, CashuWallet, type GetInfoResponse, type MintActiveKeys, type MintAllKeysets, type OutputAmounts, type Proof } from "@cashu/cashu-ts";
import { selectedDenomination, selectedNumberOfNotes, wallet } from "./stores.svelte";
...
export const getWalletWithUnit = async (mint: Mint, unit = 'sat'): Promise<CashuWallet> => {
	const keys = mint.keys.keysets.find(ks => ks.unit)
	const keysets = mint.keysets.keysets.filter(ks => ks.unit === unit)
	const wallet = new CashuWallet(new CashuMint(mint.url), { mintInfo: mint.info, unit: unit, keys, keysets })
	await wallet.getKeys()
	return wallet;
}

export const createOutputAmounts = async (denomination: number, n: number, isMint = false) => {
...
export const createOutputAmount = async (denomination: number) => {
    ...
    const availableAmounts: number[] = Object.keys(keys.keys).map(k=> parseInt(k)).sort((a, b) => b - a)
    ...
    throw new Error(`Could not create amount ${denomination} from available amounts: ${availableAmounts.join(', ')}`);
}
